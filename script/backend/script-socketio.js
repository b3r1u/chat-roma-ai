...
const { imageTeste } = require("./imageTeste.js");
...
app.get("/artwork-image", async (req, res) => {
  try {
    ...
    const randomArtwork =
      artworksWithImage[Math.floor(Math.random() * artworksWithImage.length)];
...
io.on("connection", (socket) => {
  ...
  socket.on("message", async (msg) => {
    if (msg.text.startsWith("/text ")) {
      ...
      try {
        const textResponse = await axios.post("http://localhost:4000/text", {
          userMessage,
        });
      ...
      await handleImageCommand(description, socket, io);
      ...
      try {
        ...
        const foxImageResponse = await axios.get(
          "http://localhost:4000/fox-image"
        );
